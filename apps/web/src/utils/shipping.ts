...
const ZONE_LABEL: Record<ShippingZone, string> = {
  INNER_CITY: 'Nội thành TP.HCM',
  OUTER_CITY: 'Tỉnh lân cận TP.HCM',
  PROVINCE: 'Tỉnh / TP khác',
};

/**
 * Xác định khu vực vận chuyển dựa trên địa chỉ nhận hàng.
 * Dùng để preview phí ship realtime trên frontend.
 * Backend sẽ tính lại độc lập để đảm bảo an toàn.
 */
...
/**
 * Tính phí vận chuyển.
 * Miễn phí khi subtotal (tiền hàng thuần) >= 500.000đ.
 */
...
export function getZoneLabel(zone: ShippingZone | string | null | undefined): string {
  if (!zone) return '';
